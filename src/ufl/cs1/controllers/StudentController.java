...
public final class StudentController implements DefenderController {
    ...
    private static int PPtime = 10000;
            ...
            if (possibleDirs.size() != 0) {
                ...
                if (ishungry == isHungry.EATEN) {
                    for(int j = 0; j < actions.length; j++){
                        if(distance(enemies.get(j).getLocation().getX(), pacDisX, enemies.get(j).getLocation().getY(), pacDisY) <= 25){
                            actions[j] = defender.getNextDir(pacman.getLocation(), false);
                        }
                        else{
                            actions[j] = -1;
                        }
                ...
                if (ishungry == isHungry.ONPILL) {
                        if (PPnum == 0) {
                            if(game.checkPowerPill(PP2)) {
                                ...
                                defender.getNextDir(pacman.getLocation(), false);
                            }
                    ...
                    if (i == 3) {
                            if(PPtime <= 0){
                                followObject(possibleDirs, ghostx, ghosty, pacx, pacy, pacDisX, pacDisY);
                            }
                            else{
                                actions[3] = -1;
                            }
